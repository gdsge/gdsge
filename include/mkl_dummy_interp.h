...
/* All #includes sit OUTSIDE extern "C": modern toolchain headers — notably
   gcc's <omp.h> on Apple Silicon — declare C++ templates, which are illegal
   under C linkage ("template with C linkage"). The interp routines are
   self-contained (plain double and int arguments) so they are included here too. */
#include <stdlib.h>
#include <string.h>
#ifdef USE_OMP
#include <omp.h>
...
/* -------------------------------------------------------------------------
   3) Interpolation routines (construct_cubic_spline_notaknot /
      construct_linear_interp) are included at the top of this header, outside
      the extern "C" block (see note there).
   ------------------------------------------------------------------------- */
