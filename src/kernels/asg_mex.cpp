...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	// Clear all interps' contents but KEEP allocated capacity, so the object can
	// be reused across value-iteration steps (ping-pong) instead of freshly
	// allocated + the previous one deleted. Storage reused: the hash-map buckets,
	// the surplus-pool chunks, and the vectors retain their capacity.
	if (!strcmp("reset", cmd)) {
		if (nrhs != 2) mexErrMsgTxt("reset: 2 input expected.");
		for (int i_array = 0; i_array < numArray; i_array++) {
			AsgInterp& interp = interpArray->interps[i_array];
			interp.info.clear();                       // flat_hash_map: keeps bucket capacity
			interp.gridsAll.clear();                   // vector: keeps capacity
			interp.gridsNextLevel.clear();
			interp.setOfLevelCombinations.clear();
			interp.levelSet.clear();
			interp.surplusChunkPos = 0;                // reuse the allocated surplus chunks
			interp.currentLevel = -1;
		}
		return;
	}
	...
	if (!strcmp("get_grids_info", cmd)) {
		...
		for (int i = 0; i < numArray ; i++)
		{
			...
			for (GridInfoMap::iterator it = interp.info.begin(); it != interp.info.end(); ++it)
			{
				...
				auto info = it->second;
				memcpy(p_surplus, info.surplus, nVec * sizeof(double));
	...
	if (!strcmp("reconstruct_internal", cmd)) {
		...
		for (int i_array = 0; i_array < numArray ; i_array++)
		{
			...
			for (int i_grid = 0; i_grid < nGrids ; i_grid++)
			{
				...
				infoPiece.surplus = interp.alloc_surplus(nVec);
				memcpy(infoPiece.surplus, _surplus + i_grid*nVec, sizeof(double)*nVec);
